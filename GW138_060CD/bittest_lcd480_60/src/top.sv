// 480x272 LCD 彩条显示顶层模块，RGB输出改为时序逻辑（嵌套if）
// 更新日志
//260730 V1.0.0 deepseek 初始版本（基于top.v重构，按code_style风格重写，RGB改为时序逻辑）

module top #(
    parameter PIXEL_NUM = 480               // 水平像素数
) (
    input                               i_clk,              // 系统时钟
...
////////////////////                 PLL例化                 /////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////
logic         clk_pll;      //PLL输出 71.875MHz
logic [2:0]   clk_div;      //÷8 得到约 9MHz 像素时钟
...
//Gowin PLL 的 ODIV 只能取 1/2/4/8/16/32/48/64，且 VCO 需在 700~1400MHz，
//9MHz 无法直接分出，故用 71.875MHz 再逻辑 8 分频得到 8.98MHz
always@(posedge clk_pll)begin
    clk_div <= clk_div + 3'd1;
end

assign rgb_clk   = clk_div[2];
