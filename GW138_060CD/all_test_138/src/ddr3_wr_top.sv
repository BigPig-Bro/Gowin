...
module ddr3_wr_top #(
    ...
    //第二路写口（独立像素时钟域，与第一路互不干扰，不做任何时钟切换）
    input                            i_wr_clk1,
    input                            i_wr_rst_n1,
    input                            i_wr_vs1,
    input                            i_wr_de1,
    input      [P_PIX_WIDTH-1:0]     i_wr_rgb1,
    input                            i_sel_ch1,   //1: 写第二路，0: 写第一路
...
logic [P_MEM_WIDTH-1:0]  wr_fifo0_rdata;
logic                    wr_fifo0_rd_en;
logic [WR_WNUM_W-1:0]    wr_fifo0_wr_num;
logic [WR_RNUM_W-1:0]    wr_fifo0_num;

logic [P_MEM_WIDTH-1:0]  wr_fifo1_rdata;
logic                    wr_fifo1_rd_en;
logic [WR_WNUM_W-1:0]    wr_fifo1_wr_num;
logic [WR_RNUM_W-1:0]    wr_fifo1_num;

//选择后的写FIFO接口（在ui_clk域做数据选择，时钟各自独立）
...
logic [3:0] wr_fifo0_rst_cnt;
logic       wr_fifo0_rst_n;
logic       wr_fifo0_vs_d;

always@(posedge i_wr_clk)begin
    if(!i_wr_rst_n)begin
        wr_fifo0_vs_d    <= 1'b0;
        wr_fifo0_rst_cnt <= 4'd0;
        wr_fifo0_rst_n   <= 1'b0;
    end else begin
        wr_fifo0_vs_d <= i_wr_vs;

        if(i_wr_vs && (!wr_fifo0_vs_d))begin
            wr_fifo0_rst_cnt <= P_WR_FIFO_RST_CNT;
            wr_fifo0_rst_n   <= 1'b0;
        end else if(wr_fifo0_rst_cnt != 4'd0)begin
            wr_fifo0_rst_cnt <= wr_fifo0_rst_cnt - 4'd1;
            wr_fifo0_rst_n   <= 1'b0;
        end else begin
            wr_fifo0_rst_n   <= 1'b1;
        end
...
fifo_async #(
    ...
    .i_rst_n       (wr_fifo0_rst_n),

    .i_write_clk   (i_wr_clk   ),
    .i_write_en    (i_wr_de    ),
    .i_write_data  (i_wr_rgb   ),
    .o_write_num   (wr_fifo0_wr_num),

    .i_read_clk    (i_ui_clk   ),
    .i_read_req    (wr_fifo0_rd_en),
    .o_read_data   (wr_fifo0_rdata),
    .o_data_num    (wr_fifo0_num )
);

//第二路写FIFO复位（用第二路自己的VS，时钟为第二路pclk）
logic [3:0] wr_fifo1_rst_cnt;
logic       wr_fifo1_rst_n;
logic       wr_fifo1_vs_d;

always@(posedge i_wr_clk1)begin
    if(!i_wr_rst_n1)begin
        wr_fifo1_vs_d    <= 1'b0;
        wr_fifo1_rst_cnt <= 4'd0;
        wr_fifo1_rst_n   <= 1'b0;
    end else begin
        wr_fifo1_vs_d <= i_wr_vs1;

        if(i_wr_vs1 && (!wr_fifo1_vs_d))begin
            wr_fifo1_rst_cnt <= P_WR_FIFO_RST_CNT;
            wr_fifo1_rst_n   <= 1'b0;
        end else if(wr_fifo1_rst_cnt != 4'd0)begin
            wr_fifo1_rst_cnt <= wr_fifo1_rst_cnt - 4'd1;
            wr_fifo1_rst_n   <= 1'b0;
        end else begin
            wr_fifo1_rst_n   <= 1'b1;
        end
...
fifo_async #(
    ...
    .i_rst_n       (wr_fifo1_rst_n),

    .i_write_clk   (i_wr_clk1  ),
    .i_write_en    (i_wr_de1   ),
    .i_write_data  (i_wr_rgb1  ),
    .o_write_num   (wr_fifo1_wr_num),

    .i_read_clk    (i_ui_clk   ),
    .i_read_req    (wr_fifo1_rd_en),
    .o_read_data   (wr_fifo1_rdata),
    .o_data_num    (wr_fifo1_num )
);

//写FIFO读使能按选择分配（写侧突发时只读选中的那一路）
assign wr_fifo0_rd_en = wr_fifo_rd_en && (!i_sel_ch1);
assign wr_fifo1_rd_en = wr_fifo_rd_en && ( i_sel_ch1);

//ui_clk域数据选择（时钟各自独立，不做任何时钟切换）
assign wr_fifo_rdata = i_sel_ch1 ? wr_fifo1_rdata : wr_fifo0_rdata;
assign wr_fifo_num   = i_sel_ch1 ? wr_fifo1_num   : wr_fifo0_num;
assign wr_vs_sel     = i_sel_ch1 ? i_wr_vs1       : i_wr_vs;
